...
public class ClassUtil {
    ...
    public void loadListeners(String packageName) {
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (isListener(clazz)) {
                try {
                    Neptune.getInstance().getServer().getPluginManager().registerEvents((Listener) clazz.newInstance(), Neptune.getInstance());
    ...
    public Collection<Class<?>> getClassesInPackage(String packageName) {
        ...
        CodeSource codeSource = Neptune.getInstance().getClass().getProtectionDomain().getCodeSource();
